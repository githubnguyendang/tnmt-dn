...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const accessPermission = async (linkControl: string, action: string): Promise<boolean> => {
    ...
    // Check if the first character of linkControl is '/' and remove it if true
    const formattedLinkControl = linkControl.startsWith('/') ? linkControl.slice(1) : linkControl
    ...
    try {
      const response = await axios.post(
        ...
        {},
        {
          params: {
            userName: user,
            linkControl: formattedLinkControl,
            action
          },
          headers: {
