...
const VerticalNavLink = ({ item, settings, navVisible, toggleNavVisibility }: Props) => {
  ...
  const isNavLinkActive = useCallback(
    (itemPath: string | undefined) => {
      return router.pathname === itemPath || handleURLQueries(router, itemPath)
    },
    [router]
  )
  ...
  }, [isNavLinkActive, item.children])

  if (item.children && item.children.length > 0) {
    // Level 1 Menu Item with children
    return (
      <ListItem disablePadding className='nav-link' sx={{ px: '0 !important', display: `block` }}>
