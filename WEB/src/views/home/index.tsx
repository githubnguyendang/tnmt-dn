...
const Home = () => {
  ...
  return (
    <Grid container spacing={6} px={25}>
      <Grid item md={12} display={'flex'} alignItems={'center'}></Grid>
      ...
      <Grid item md={12}>
        <Grid container spacing={5} justifyContent={'center'}>
              ...
              {navigationItems.map((item, index) => {
                return (
                  <Grid
                    ...
                    <Button href={item.path} fullWidth sx={{ p: 0 }}>
                      <Card
                        sx={{
                          display: 'flex',
                        ...
                        <CardMedia component='img' sx={{ width: 50, mr: 4 }} image={item.logo} alt={item.name} />
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignContent: 'center' }}>
                          <CardContent sx={{ flex: '1 0 auto', p: '.5rem !important' }}>
                            <Typography component='div' variant='subtitle1' fontWeight={500} fontSize={16}>
                              {item.name}
                            </Typography>
                          </CardContent>
                        </Box>
          ...
          <Grid item md={7}>
            <img src='/images/bandohanhchinh.png' alt='' width={'50%'} />
          </Grid>
        </Grid>
      </Grid>
      <Grid item md={12} alignContent={'center'}>
        <Box sx={{ width: '100%', textAlign: 'center' }}>
          <Typography variant='caption' fontWeight={700} color='#ff3a3f'>
            Bản quyền ©2024 thuộc Sở Tài nguyên và Môi trường Quảng Ngãi |{' '}
          </Typography>
