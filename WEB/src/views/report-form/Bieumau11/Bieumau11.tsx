      ...
      <Grid xs={12} md={12}>
        ...
        <Paper elevation={3} sx={{ p: 5, height: '100%' }}>
          <Box sx={{ width: 'max-content', p: 3 }}>
            <ExportTableToExcel tableId={'BieuMau11'} filename={'BieuMau11'} />
          </Box>
          <TableComponent columns={columnsTable} rows={data} id='BieuMau11' loading={loading} />
          <FooterReport />
        </Paper>
      </Grid>
    </Grid>
  ...
  return (
    <DialogControlFullScreen>
      {(openDialogs: (content: React.ReactNode, title: React.ReactNode) => void) => (
        <Box className='formReport_box' onClick={() => openDialogs(<BieuMauMuoiMot />, formTitle)}>
          <Grid item xs={8}>
