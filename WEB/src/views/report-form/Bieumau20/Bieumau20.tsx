    ...
    },
    {
      ...
      label: (
        <Box>
          ...
          (hồ, đập, cống, <br />
          trạm bơm, giếng khoan,khác)
        </Box>
      ),
      align: 'left',
      minWidth: 200,
      elm: (row: any) => <Typography>{row?.loaiCT.tenLoaiCT}</Typography>
    },
    {
      id: 'nguonNuocKT',
      label: (
      ...
      align: 'left',
      minWidth: 250,
      elm: (row: any) => <Typography>{row?.nguonNuocKT}</Typography>
    },

    {
      ...
      children: [
        ...
        },
        {
          id: 'tinh',
          label: 'Tỉnh',
          elm: () => <Typography>Quảng Ngãi</Typography>,
          minWidth: 150
        }
      ]
    },
          ...
          children: [
            {
              ...
              elm: (row: any) => <Typography>{row?.thongso?.dungTichToanBo}</Typography>
            },
            {
              id: 'dungTichHuuIch',
              label: (
                <Box>
                  Dung tích <br /> hữu ích <br /> (triệu m3)
                </Box>
              ),
              elm: (row: any) => <Typography>{row?.thongso?.dungTichHuuIch}</Typography>
            },
            {
              id: 'congSuatDamBao',
              label: (
              ...
              elm: (row: any) => <Typography>{row?.thongso?.congSuatLM}</Typography>
            }
          ]
        },
        {
          ...
          children: [
            {
              ...
              label: (
                <Box>
                  Lưu lượng <br /> thiết kế <br /> (m3/ngày đêm)
                </Box>
              ),
              elm: (row: any) => (
                <Typography>
                  {row?.idLoaiCT !== 4 && row?.idLoaiCT !== 5 && row?.idLoaiCT !== 11
                    ? row?.tongLuuLuong != 0
                      ? row?.tongLuuLuong
                      : null
                    : null}
            ...
            },
            {
              ...
              label: (
                <Box>
                  Lưu lượng <br /> thực tế <br /> (m3/ngày đêm)
                </Box>
              ),
              elm: (row: any) => (
                <Typography>
    ...
    <Grid container spacing={2}>
      <Grid xs={12} md={12}>
        ...
        <Paper elevation={3} sx={{ p: 5, height: '100%' }}>
          <Box sx={{ width: 'max-content', p: 3 }}>
            <ExportTableToExcel tableId={'BieuMau20'} filename={'BieuMau20'} />
          </Box>
          <TableComponent
  ...
  return (
    <DialogControlFullScreen>
      {(openDialogs: (content: React.ReactNode, title: React.ReactNode) => void) => (
        <Box className='formReport_box' onClick={() => openDialogs(<BieuMauHaiMuoi />, formTitle)}>
          <Grid item xs={8}>
