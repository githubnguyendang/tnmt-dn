...
const FormContruction = () => {
  ...
  const columnsTable: TableColumn[] = [
    ...
    },
    {
      ...
      label: (
        <Box>
          Lượng nước khai thác, sử dụng (quy mô) <br /> đã được cấp giấy phép khai thác tài nguyên nước
        </Box>
      ),
      children: [
        {
        ...
        {
          id: 'mucdichkhac',
          label: 'Mục đích khác(m³/Ngày đêm)',
          children: [
      ...
      {loading ? (
        ...
        <Grid className='_text_center' sx={{ mt: 3 }}>
          <Box sx={{ width: 'max-content' }}>
            <ExportTableToExcel tableId={'bieumau12'} filename={'bieumau12'} />
          </Box>
          <TableComponent columns={columnsTable} id='bieumau12' rows={data} loading={loading} />
        </Grid>
      )}
      <FooterReport />
    </Paper>
        ...
        <Box className='formReport_box' onClick={() => openDialogs(<FormContruction />, formTitle)}>
          <Grid item xs={8}>
            ...
            <Typography className='text-success text-weight-bold _font12'>
              Lượng nước KTSD đã được cấp giấy phép khai thác tài nguyên nước
            </Typography>
          </Grid>
          <Grid item xs={4}>
