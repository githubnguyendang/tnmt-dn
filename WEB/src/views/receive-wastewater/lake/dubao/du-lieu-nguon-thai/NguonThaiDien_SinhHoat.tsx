...
import ThaiSinhHoatDBForm from './form/NguonThaiSinhHoatForm'

const MapDoanSong = dynamic(() => import('src/@core/components/map/mapdoansong'), { ssr: false })

const NguonThaiSinhHoatDB = () => {
  ...
  useEffect(() => {
    async function getDataReport1() {
      setLoading(true)
      await getData('DuLieuNguonNuocThaiSinhHoatDB/danh-sach')
        .then(data => {
          setData(data)
        })
        .catch(error => {
      ...
      <Grid item xs={12}>
        <Paper elevation={3} sx={{ p: 0, height: '100%' }}>
          <Grid container className='_flexEnd' spacing={2} sx={{ p: 2 }}>
            ...
            <Grid item>
              <ThaiSinhHoatDBForm isEdit={false} setPostSuccess={handlePostSuccess} />
            </Grid>
          </Grid>
          <TableComponent
            ...
            actions={(row: any) => (
              <Box className='group_btn'>
                <ThaiSinhHoatDBForm isEdit={true} data={row} setPostSuccess={handlePostSuccess} />
...
export default NguonThaiSinhHoatDB
