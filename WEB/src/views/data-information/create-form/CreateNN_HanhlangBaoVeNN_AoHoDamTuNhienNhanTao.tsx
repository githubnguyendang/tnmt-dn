...
const Form = ({ data, setPostSuccess, closeDialogs }: any) => {
  const [NN_HanhLangBaoVeNN_AoHoDamTuNhienNhanTaoData, setNN_HanhLangBaoVeNN_AoHoDamTuNhienNhanTaoData] =
    useState<State>({
      id: data?.id || 0,
      ten: data?.ten || '',
    ...
    const handleApiCall = async () => {
      ...
      try {
        const res = await saveData(
          'NN_HanhLangBaoVeNN_AoHoDamTuNhienNhanTao/luu',
          NN_HanhLangBaoVeNN_AoHoDamTuNhienNhanTaoData
        )
        if (res) {
          // Reset form fields
