...
import { Grid, Button, TextField, DialogActions, Box } from '@mui/material'
...
const Form = ({ user, setPostSuccess, closeDialogs }: any) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const res = await saveData(`Auth/set-password?username=${user.userName}&newPassword=${newPassword}`, {})
      ...
      {(openDialogs: (content: React.ReactNode, title: React.ReactNode) => void, closeDialogs: () => void) => (
        <Box>
          <LockOpen
            ...
            sx={{ marginRight: 2 }}
            onClick={() =>
              openDialogs(<Form user={user} setPostSuccess={setPostSuccess} closeDialogs={closeDialogs} />, formTitle)
            }
          />
        </Box>
      )}
