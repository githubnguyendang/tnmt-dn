...
interface MiningPurposeFieldProps {
  ...
  type?: any
  onChange: (data: MiningPurposeState[]) => void
}

const MiningPurpose: FC<MiningPurposeFieldProps> = ({ data, type, onChange }) => {
  const [miningPurposes, setMiningPurposes] = useState<MiningPurposeState[]>([])
  ...
  const handleOpenModal = (index: number, e: any, func: 'add' | 'update') => {
    ...
    if (func === 'add') {
      ...
      const nullValue = Object.fromEntries(Object.keys(newMiniPurpose || {}).map(key => [key, null]))
  ...
  const handleSave = () => {
    if (newMiniPurpose.idMucDich !== undefined) {
      if (newMiniPurposeIndex >= 0) {
        ...
        setMiningPurposes(prevItems => {
          const updatedItems = [...prevItems, newMiniPurpose]
  ...
  useEffect(() => {
    if (data) {
      const initialData = data.map((e: MiningPurposeState) => ({
        id: e.id || undefined,
      ...
      console.log('Initialized mining purposes:', initialData)
      setMiningPurposes(initialData)
    }
  }, [data])
    ...
    <fieldset>
      <legend>
        <Typography variant={'subtitle1'} className='legend__title'>
          {type == 1 || type == 7
            ? 'Lưu lượng theo mục đích khai thác sử dụng'
            : type == 8
            ? 'Lưu lượng theo mục đích thăm dò'
            : type == 3
            ? 'Lưu lượng theo mục đích xả thải'
            : ''}
        </Typography>
      </legend>
                      ...
                      <Box sx={{ ...style, width: 600 }}>
                        <Typography id='transition-modal-title' variant='h6' component='h2' align='center' py={3}>
                          {type == 1 || type == 2 || type == 7 || type == 8 || type == 9
                            ? 'LƯU LƯỢNG THEO MỤC ĐÍCH KHAI THÁC'
                            : type == 3
                            ? 'LƯU LƯỢNG THEO MỤC ĐÍCH XẢ THẢI'
                            : 'LƯU LƯỢNG THEO MỤC ĐÍCH'}
                        </Typography>
                        {required ? (
                        ...
                        <Grid container spacing={4}>
                          <Grid item md={12}>
                            <Autocomplete
                              ...
                              value={
                                listMiningPurposes.find((option: any) => option.id === newMiniPurpose?.idMucDich) ||
                                null
                              }
            ...
            {miningPurposes
              ...
              .map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{item.luuLuong}</TableCell>
                  <TableCell>{item.donViDo}</TableCell>
