...
const GroundWaterField: FC<ConsTypeFieldsetProps> = ({ props, onChange }) => {
      ...
      try {
        ...
        const filteredData =
          GetConstructionTypeId(router) === 7
            ? consTypes.filter((item: any) => item.id === 7 || item.id === 10)
            : consTypes.filter((item: any) => item.idCha === GetConstructionTypeId(router))
  ...
  const handleSetCons = (data: ConstructionState) => {
    const cons: ConstructionState = data
    setShowDataCons(true)
    setConstruction({ ...cons })

    // Cập nhật trạng thái cho quận và xã
    const selectedDistricts = cons.vitri?.map((location: any) => `${location.tenHuyen}-${location.idHuyen}`) || []
    ...
    // Gọi onChange với dữ liệu đã cập nhật
    ...
    <Box>
      <fieldset>
        ...
        <Grid container spacing={6}>
          {loai_ct == 'tham-do' || loai_ct == 'hanh-nghe-khoan' ? (
            ''
          ) : (
            ...
            <Grid item xs={12} md={4} sm={12}>
              <Autocomplete
                ...
                getOptionLabel={(option: any) =>
                  `${option.id}, ${option.tenCT} ${option.huyen !== null ? `(${option.huyen[0]?.tenHuyen})` : ''}`
                }
                value={
                  ds_congtrinh.find(
      ...
      {showDataCons ? (
        <Grid item xs={12}>
          ...
          <ConstructionItem
            ...
            type={isLicensepage ? GetConstructionTypeId(router) : construction.idLoaiCT}
