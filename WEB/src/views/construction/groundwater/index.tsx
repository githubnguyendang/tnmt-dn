...
const GroundConstruction = () => {
  ...
  const columnsTable: TableColumn[] = [
    ...
    },
    {
      ...
      pinned: 'left',
      minWidth: 350,
      elm: (row: any) => <Typography>{row?.giayphep[0]?.tochuc_canhan?.tenTCCN}</Typography>
    },
    {
      id: 'viTriCT',
      label: 'Địa điểm',
    ...
    { id: 'mucDichKT', label: 'Mục đích khai thác', align: 'left', minWidth: 300 },
    {
      ...
      align: 'left',
      elm: (row: any) => row?.thongso?.soLuongGiengKT
    },
    {
      id: 'cheDoKT',
    ...
    { id: 'thoiGianHNK', label: 'Thời gian hành nghề khoan', align: 'left' },
    {
      ...
      children: [
        {
          id: 'chieuSauDoanThuNuocTu',
          label: 'Từ',
          align: 'left',
          elm: (row: any) => row?.thongso?.chieuSauDoanThuNuocTu
        },
        {
          id: 'chieuSauDoanThuNuocDen',
          label: 'Đến',
          align: 'left',
          elm: (row: any) => row?.thongso?.chieuSauDoanThuNuocDen
        }
      ]
    },
    {
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          elm: (row: any) => (
            <div style={{ width: '100%' }}>
              {row.giayphep?.map((e: any) => e.tiencq.map((c: any) => <div key={c.id}>{formatNum(c.tongTienCQ)}</div>))}
            </div>
          )
        }
      ]
