...
const LicenseFeeFeild: FC<LicenseFeeFieldsetProps> = ({ data, onChange }) => {
  const initialLicenseFees: LicenseFeeState[] = data
    ? data.map((e: LicenseFeeState) => ({
        id: e.id,
        idCon: e.idCon,
        soQDTCQ: e.soQDTCQ,
  ...
  const handleOpenModal = (index: number, e: any, func: 'add' | 'update') => {
    ...
    if (func === 'add') {
      ...
      const nullValue = Object.fromEntries(Object.keys(newLicenseFee || {}).map(key => [key, null]))
                    ...
                    <Fade in={openModal}>
                      <Box sx={{ ...style, width: 600 }}>
                        ...
                        <Grid container spacing={4}>
                          ...
                          <Grid item md={12}>
                            {newLicenseFee.fileUpload && (
                              <Typography mb={3}>{newLicenseFee.fileUpload?.name}</Typography>
                            )}
                            <Button
                              fullWidth
                              className='uploadBtn'
          ...
          <TableBody>
            {licenseFees.map((item, index) => (
              <TableRow key={index}>
                ...
                <TableCell>{item.soQDTCQ}</TableCell>
                <TableCell>{formatDate(item.ngayKy)}</TableCell>
                <TableCell>{item.tongTienCQ}</TableCell>
